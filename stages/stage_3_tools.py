    ...
    for i, job in enumerate(jobs, 1):
        ...
        if resume_snippet:
            analysis = analyze_skills_gap(
                resume_snippet, job["title"] + " " + job["snippet"]
            )
            lines.append(f"   Match Score: {analysis['match_score']}%")
            lines.append(
                f"   Matching Skills: {', '.join(analysis['matching_skills']) if analysis['matching_skills'] else 'None'}"
            )
            lines.append(
                f"   Missing Skills: {', '.join(analysis['missing_skills']) if analysis['missing_skills'] else 'None'}"
            )
...
Based on these real listings, what should I do next?"""
    ...
    return {"jobs_text": jobs_text, "response": response}
